...
async function checkNewTransactions(walletAddress, onTransaction) {
  try {
    ...
    for (const tx of txs) {
      ...
      // Skip txs older than bot startup UNLESS they match a pending order
      if (txTime < monitorStartTime) {
        const hasPendingOrder = Object.values(storeState.get().pendingOrders).some(
          order => Math.abs(order.ltcAmount - (tx.outputs || []).reduce((sum, o) => {
            if (o.addresses?.includes(walletAddress)) return sum + o.value;
            return sum;
          }, 0) / LTC_SATOSHI) <= 0.00005
        );
        if (!hasPendingOrder) {
          // Mark as seen so we never log it as unmatched
          storeState.markTxSeen(tx.hash, '_unmatched_logged');
...
function matchOrderByLtcAmount(ltcAmount) {
  const state = storeState.get();
  const EXACT_TOLERANCE = 0.00005;  // exact match tolerance
  const LOOSE_TOLERANCE = 0.005;    // loose match for overpay/underpay (±0.5%)
  ...
  for (const [orderId, order] of pending) {
    ...
    if (diff <= EXACT_TOLERANCE) {
      console.log(`  ✅ Exact match → ${orderId} (${order.gamblitUsername})`);
      return { orderId, order, overpaid: 0, underpaid: 0 };
    }
  }

  // Second pass: loose match for overpay/underpay
  for (const [orderId, order] of pending) {
    const diff = ltcAmount - order.ltcAmount; // positive = overpaid, negative = underpaid
    if (Math.abs(diff) <= LOOSE_TOLERANCE) {
      if (diff > 0) {
        console.log(`  ✅ Overpaid match → ${orderId} (+${diff.toFixed(6)} LTC extra)`);
      } else {
        console.log(`  ⚠️ Underpaid match → ${orderId} (${diff.toFixed(6)} LTC short)`);
      }
      return { orderId, order, overpaid: diff > 0 ? diff : 0, underpaid: diff < 0 ? Math.abs(diff) : 0 };
    }
  }

  // Third pass: find closest order (might be significantly wrong amount)
  if (pending.length > 0) {
    const closest = pending.reduce((best, [orderId, order]) => {
      const diff = Math.abs(order.ltcAmount - ltcAmount);
      return diff < best.diff ? { orderId, order, diff } : best;
    }, { diff: Infinity });

    if (closest.diff < 0.01) { // within 0.01 LTC
      const diff = ltcAmount - closest.order.ltcAmount;
      console.log(`  ⚠️ Close match → ${closest.orderId} (diff=${closest.diff.toFixed(6)} LTC)`);
      return { orderId: closest.orderId, order: closest.order, overpaid: diff > 0 ? diff : 0, underpaid: diff < 0 ? Math.abs(diff) : 0 };
    }
  }
...
// Look up a specific TX hash and return how much LTC was sent to our wallet
async function lookupTx(txHash, walletAddress) {
  try {
    const token = process.env.BLOCKCYPHER_TOKEN;
    const query = token ? `?token=${token}` : '';
    const url = `https://api.blockcypher.com/v1/ltc/main/txs/${txHash}${query}`;
    const res = await axios.get(url, { timeout: 15000 });
    const tx = res.data;
    ...
    return {
      found: true,
      ltcAmount: receivedSatoshis / 100_000_000,
