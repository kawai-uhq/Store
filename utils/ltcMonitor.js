...
const CONFIRMATIONS_NEEDED = 6; // Fire tip at 6 confirmations
  ...
  try {
    ...
    for (const tx of txs) {
      ...
      if (tx.confirmations >= 6) {
        // Fully confirmed — fire once for tip processing
        if (!seenTxHashes.has(tx.hash)) {
          seenTxHashes.add(tx.hash);
          seenTxHashes.add(tx.hash + '_1conf');
        ...
        // 1+ confirmations — fire for hold (once per tx)
        if (!seenTxHashes.has(tx.hash + '_1conf')) {
          seenTxHashes.add(tx.hash + '_1conf');
          await onTransaction({ txHash: tx.hash, ltcAmount, confirmations: tx.confirmations, pending: false });
        }
      } else if (!seenTxHashes.has(tx.hash + '_hold')) {
        // Unconfirmed — notify user
        seenTxHashes.add(tx.hash + '_hold');
