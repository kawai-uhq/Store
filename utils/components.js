  ...
  const container = new ContainerBuilder()
    ...
    .addTextDisplayComponents(
      new TextDisplayBuilder().setContent(
        `**Instructions:**\n` +
        `1. Press 🛒 **Buy** below\n` +
        `2. Enter your Gamblit username + how much LTC you want to send\n` +
        `3. Send that LTC to our wallet address\n` +
        `4. Click **Submit TX ID** and paste your transaction ID\n` +
        `5. BGLs arrive automatically — you get the full equivalent!`
      )
    )
    .addSeparatorComponents(
      new SeparatorBuilder().setSpacing(SeparatorSpacingSize.Small).setDivider(true)
...
// ─── ORDER CONFIRMATION ───────────────────────────────────────────────────────
function buildOrderConfirmation({ orderId, bglAmount, ltcAmount, walletAddress, currency, fiatAmount, expiresAt, rateNote }) {
  ...
  const container = new ContainerBuilder()
    ...
    .addTextDisplayComponents(
      new TextDisplayBuilder().setContent(
        `🎮 **Estimated BGLs:** ~${bglAmount}\n` +
        `💎 **LTC to send:** \`${ltcAmount}\` LTC\n` +
        `💰 **Value:** ${currSymbol}${parseFloat(fiatAmount).toFixed(2)}\n` +
        (rateNote ? `-# ${rateNote}` : '')
      )
    )
    .addSeparatorComponents(
    ...
    .addTextDisplayComponents(
      new TextDisplayBuilder().setContent(
        ...
        `\n✅ You will receive BGLs equivalent to however much LTC you send at current rates.\n` +
        `📨 After sending, click **Submit TX ID** below.\n` +
