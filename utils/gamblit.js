...
async function getBalanceBgl() {
  try {
    ...
    const wlBalance = await page.evaluate(() => {
      // The balance shows as "71,473" next to the lock icon in the top navbar
      // It's always the number right next to the WL lock icon (🔒)

      // Strategy 1: find the element containing the balance number near the lock icon
      // Look for numbers formatted with commas like "71,473" or plain like "22049"
      ...
      for (const el of allEls) {
        ...
        // Match numbers with optional commas: 71,473 or 22049 or 220.49
        if (/^\d{1,3}(,\d{3})*(\.\d+)?$/.test(text) || /^\d+(\.\d+)?$/.test(text)) {
          const num = parseFloat(text.replace(/,/g, ''));
          if (num > 0) candidates.push({ num, text, el: el.outerHTML.slice(0, 80) });
        }
      }

      // Log all candidates for debugging
      console.log('Balance candidates:', JSON.stringify(candidates.slice(0, 10)));

      // The balance is usually the largest number in the header area
      // Filter to numbers that are plausibly a WL balance (> 100)
      const plausible = candidates.filter(c => c.num >= 1);
      if (!plausible.length) return null;

      // Sort by position in DOM — balance is near the top of the page
      // Return the first number that appears in the header/nav area
      const header = document.querySelector('header, nav, [class*="header"], [class*="navbar"]');
      if (header) {
        for (const c of plausible) {
          // Check if this element is inside the header
          const allHeaderText = header.innerText || '';
          if (allHeaderText.includes(c.text)) return c.num;
        }
      }

      // Fallback: return the largest candidate
      return Math.max(...plausible.map(c => c.num));
    });
    ...
    if (!wlBalance) {
      ...
      console.warn('[Gamblit] Could not find balance — check /tmp/gamblit_balance_debug_*.png');
      return null;
    }

    // 220.49 DL = 22,049 WL → but page may show DL directly
    // Check if the number is likely DL (small, like 220.49) or WL (large, like 22049)
    let wl, dl, bgl;
    if (wlBalance < 1000) {
      // Probably showing DL balance directly
      dl = wlBalance;
      wl = dl * 100;
    } else {
      // Showing WL balance
    ...
    bgl = Math.floor((dl / DL_PER_BGL) * 100) / 100;

    console.log(`[Gamblit] Balance: ${wl.toLocaleString()} WL = ${dl.toFixed(2)} DL = ${bgl} BGLs`);
    return { wl, dl, bgl };
  } catch (e) {
