...
async function tipUser(growId, bglAmount) {
  ...
  const userIconClicked = await page.evaluate(() => {
    // The user icon is a person/profile icon in the header
    // Try SVG icons, img tags, and buttons that look like user icons
    const candidates = [
    ...
    for (const el of candidates) {
      ...
      // Match user/profile/person icons
      if (
        html.includes('person') || html.includes('user') || html.includes('profile') ||
        html.includes('account') || cls.includes('user') || cls.includes('profile') ||
        cls.includes('account') || aria.includes('user') || aria.includes('profile') ||
        aria.includes('tip') || aria.includes('send')
      ) {
        el.click();
    ...
    // Try clicking SVG path that looks like a person silhouette
    const svgs = document.querySelectorAll('svg');
    for (const svg of svgs) {
      const parent = svg.closest('button, a, [role="button"]');
      if (parent) {
        parent.click();
        return 'svg-parent: ' + parent.outerHTML.slice(0, 80);
      }
    }
    return null;
  });
