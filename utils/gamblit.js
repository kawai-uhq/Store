...
async function getBalanceBgl() {
  try {
    ...
    const wlBalance = await page.evaluate(() => {
      // Navbar shows WL balance like "22,046" inside the pill next to lock icon + "+" button
      // Find the + button then read the number from its parent pill container
      ...
      if (plusBtn) {
        ...
        for (let i = 0; i < 5; i++) {
          ...
          // Match integers (WL balances like 22046) or decimals (DL like 220.46)
      ...
      // Fallback: scan header for large numbers
    ...
    console.log('[Gamblit] Raw balance from page:', wlBalance);
    ...
    // Determine if value is WL (integer like 22046) or DL (decimal like 220.46)
    let wl, dl;
    if (Number.isInteger(wlBalance) || wlBalance > 1000) {
      // WL format: 22,046 WL
      wl = wlBalance;
      dl = wl / 100;
    } else {
      // DL format: 220.46 DL
      dl = wlBalance;
      wl = Math.round(dl * 100);
    }

    const bgl = Math.floor((wl / WL_PER_BGL) * 100) / 100;
