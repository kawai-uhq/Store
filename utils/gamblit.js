...
async function tipUser(growId, bglAmount) {
  ...
  // ── Step 1: Click the user/person icon to open tip modal ─────────────────
  // From screenshot: layout is lock+balance, + button, person icon, trophy, gift, chat
  // The person icon is the first clickable element after the + button
  const userIconClicked = await page.evaluate(() => {
    // Find the + button first, then get the next sibling clickable element
    const allClickable = [...document.querySelectorAll('button, a, [role="button"]')];

    // Find + button by text content
    const plusBtn = allClickable.find(el => el.textContent.trim() === '+');
    if (plusBtn) {
      // Get the next clickable element after the + button
      let next = plusBtn.nextElementSibling;
      while (next) {
        if (next.tagName === 'BUTTON' || next.tagName === 'A' || next.getAttribute('role') === 'button' || next.querySelector('svg')) {
          next.click();
          return 'next-after-plus: ' + next.outerHTML.slice(0, 100);
        }
        // Check children
        const btn = next.querySelector('button, a, [role="button"]');
        if (btn) { btn.click(); return 'child-after-plus: ' + btn.outerHTML.slice(0, 100); }
        next = next.nextElementSibling;
      }
    }

    // Fallback: find by href containing user/profile/tip
    const links = [...document.querySelectorAll('a[href*="tip"], a[href*="user"], a[href*="profile"], a[href*="send"]')];
    if (links.length) { links[0].click(); return 'link: ' + links[0].href; }

    // Fallback: click the 2nd SVG icon on the page (after balance area)
    const svgParents = [...document.querySelectorAll('svg')].map(s => s.closest('button, a, [role="button"]')).filter(Boolean);
    const unique = [...new Set(svgParents)];
    if (unique.length >= 2) { unique[1].click(); return '2nd-svg-parent'; }
