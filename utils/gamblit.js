...
async function tipUser(growId, bglAmount) {
  ...
  const tipMenuClicked = await page.evaluate(() => {
    // Find the Tip link in the dropdown — it shows as "Tip" text with a coin icon
  ...
  // Coordinate fallback — Tip is at approximately x=1137, y=214 in the dropdown
  if (!tipMenuClicked || tipMenuClicked === 'not-found') {
    console.log('[Gamblit] Using coordinate click for Tip menu item');
