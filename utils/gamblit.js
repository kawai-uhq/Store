...
async function tipUser(growId, bglAmount) {
  ...
  // ── Step 1: Click the user/person icon ───────────────────────────────────
  // Screenshot shows: gambl logo, lock+71473, + button, 👤 person icon, trophy, gift, chat
  // Person icon is directly after the + button in the DOM
  const userIconClicked = await page.evaluate(() => {
    const allClickable = [...document.querySelectorAll('button, a, [role="button"], [onclick]')];

    // Strategy 1: element right after + button
    const plusIndex = allClickable.findIndex(el => el.textContent.trim() === '+');
    if (plusIndex !== -1 && allClickable[plusIndex + 1]) {
      allClickable[plusIndex + 1].click();
      return 'after-plus: ' + allClickable[plusIndex + 1].outerHTML.slice(0, 80);
    }

    // Strategy 2: SVG person icon — look for path data resembling a person
    const allSvgs = [...document.querySelectorAll('svg')];
    for (const svg of allSvgs) {
      const pathD = svg.querySelector('path')?.getAttribute('d') || '';
      // Person icons typically have circular head + body shape
      if (pathD.includes('M12') || pathD.includes('circle') || svg.innerHTML.includes('person')) {
        const parent = svg.closest('button, a, [role="button"]') || svg.parentElement;
        if (parent) { parent.click(); return 'person-svg: ' + parent.outerHTML.slice(0, 80); }
      }
    }

    // Strategy 3: click by coordinates — person icon is at roughly x=16, y=400 based on screenshot
    return 'no-match';
  });

  console.log('[Gamblit] Person icon click result:', userIconClicked);

  // If JS click didn't work, try clicking by coordinate (person icon position from screenshot)
  if (!userIconClicked || userIconClicked === 'no-match') {
    console.log('[Gamblit] Trying coordinate click for person icon...');
