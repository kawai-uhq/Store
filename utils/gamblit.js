...
async function getBalanceBgl() {
  try {
    ...
    const dlBalance = await page.evaluate(() => {
      // The balance is shown in the header as a number like "714.73"
      // Try to find it near the + button or wallet icon
      ...
      for (const el of allEls) {
        ...
        if (!text) continue;
        // Match a decimal number that looks like a balance (e.g. 714.73)
    ...
    const bglBalance = Math.floor(dlBalance / DL_PER_BGL);
...
module.exports = { tipUser, verifyToken, getBalanceBgl, closeBrowser, TIP_DAILY_LIMIT, DL_PER_BGL };
