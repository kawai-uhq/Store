...
// 15-second tip cooldown tracker
let lastTipTime = 0;
async function waitForTipCooldown() {
  const now = Date.now();
  const elapsed = now - lastTipTime;
  const COOLDOWN_MS = 16000; // 16s to be safe
  if (elapsed < COOLDOWN_MS) {
    const wait = COOLDOWN_MS - elapsed;
    console.log(`[TipCooldown] Waiting ${(wait/1000).toFixed(1)}s before next tip...`);
...
async function processConfirmedPayment(txData, client) {
  ...
  if (confirmations < 6) {
    console.log(`[OrderProcessor] Order ${orderId}: ${confirmations}/6 confirmations`);
    return;
  }

  console.log(`[OrderProcessor] 6 confirmations ✅ — processing tip for ${orderId}`);
  storeState.completeOrder(orderId, txHash);

  try {
    // Respect 15s cooldown between tips
    await waitForTipCooldown();
