...
// Process LTC payment — called by ltcMonitor or handleTxModalSubmit
async function processConfirmedPayment(txData, client) {
  const { txHash, ltcAmount, confirmations, pending } = txData;

  // Match by txHash (set when user submits TX ID)
  const state = storeState.get();
  let orderId, order;

  const txMatch = Object.entries(state.pendingOrders).find(([, o]) => o.txHash === txHash);
  if (txMatch) {
    [orderId, order] = txMatch;
  } else {
    // Fallback: match by LTC amount (for backwards compatibility)
    const amountMatch = ltcMonitor.matchOrderByLtcAmount(ltcAmount);
    if (amountMatch) {
      ({ orderId, order } = amountMatch);
    }
  }
  ...
  if (!orderId || !order) {
    if (!storeState.isTxSeen(txHash, '_unmatched_logged')) {
      ...
      console.warn(`[OrderProcessor] No order match for tx: ${txHash}`);
    }
    return;
  }
